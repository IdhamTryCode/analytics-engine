...
import io.analytics.main.web.dto.DryPlanDtoV2;
import io.analytics.main.web.dto.ErrorMessageDto;
...
import io.analytics.main.web.dto.SqlAnalysisInputBatchDto;
...
public abstract class RequireAnalyticsServer
{
    protected TestingAnalyticsServer analyticsServer;
    ...
    public RequireAnalyticsServer() {}

    @BeforeClass
    public void init()
            throws Exception
    {
        this.analyticsServer = createAnalyticsServer();
        this.client = closer.register(createHttpClient());
        closer.register(analyticsServer);
        prepare();
    }
    ...
    protected abstract TestingAnalyticsServer createAnalyticsServer()
            throws Exception;
    ...
    protected void initDuckDB()
    {
        ...
        DuckDBMetadata metadata = analyticsServer.getInstance(Key.get(DuckDBMetadata.class));
        metadata.setInitSQL(initSQL);
        metadata.reload();
    }

    protected TestingAnalyticsServer server()
    {
    ...
    {
        return analyticsServer.getInstance(key);
    }
