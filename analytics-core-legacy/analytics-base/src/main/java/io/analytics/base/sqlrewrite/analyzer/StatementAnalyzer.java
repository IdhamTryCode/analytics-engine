...
public final class StatementAnalyzer
{
    ...
    public static Scope analyze(Analysis analysis, Statement statement, SessionContext sessionContext, AnalyticsMDL analyticsMDL)
    {
        return analyze(analysis, statement, sessionContext, analyticsMDL, Optional.empty());
    }

    public static Scope analyze(Analysis analysis, Statement statement, SessionContext sessionContext, AnalyticsMDL analyticsMDL, Optional<Scope> scope)
    {
        Scope queryScope = new Visitor(sessionContext, analysis, analyticsMDL).process(statement, scope);
        // add models directly used in sql query
        analysis.addModels(
                analyticsMDL.listModels().stream()
                        .filter(model -> analysis.getTables().stream()
                                .filter(table -> table.getCatalogName().equals(analyticsMDL.getCatalog()))
                                .filter(table -> table.getSchemaTableName().getSchemaName().equals(analyticsMDL.getSchema()))
            ...
            extends AstVisitor<Scope, Optional<Scope>>
    {
        ...
        public Visitor(
                ...
                Analysis analysis,
                AnalyticsMDL analyticsMDL)
        {
            this.sessionContext = requireNonNull(sessionContext, "sessionContext is null");
            this.analysis = requireNonNull(analysis, "analysis is null");
            this.analyticsMDL = requireNonNull(analyticsMDL, "analyticsMDL is null");
        }
        ...
        protected Scope visitTable(Table node, Optional<Scope> scope)
        {
            ...
            Scope outputScope;
            if (tableName.getCatalogName().equals(analyticsMDL.getCatalog()) && tableName.getSchemaTableName().getSchemaName().equals(analyticsMDL.getSchema())) {
        ...
        {
            if (analyticsMDL.getModel(tableName.getSchemaTableName().getTableName()).isPresent()) {
                return analyticsMDL.getModel(tableName.getSchemaTableName().getTableName())
            ...
            else if (analyticsMDL.getMetric(tableName.getSchemaTableName().getTableName()).isPresent()) {
                return analyticsMDL.getMetric(tableName.getSchemaTableName().getTableName())
            ...
            else if (analyticsMDL.getCumulativeMetric(tableName.getSchemaTableName().getTableName()).isPresent()) {
                CumulativeMetric cumulativeMetric = analyticsMDL.getCumulativeMetric(tableName.getSchemaTableName().getTableName()).get();
        ...
        protected Scope visitFunctionRelation(FunctionRelation node, Optional<Scope> scope)
        {
            if (node.getName().toString().equalsIgnoreCase("roll_up")) {
                ...
                Metric metric = analyticsMDL.getMetric(catalogSchemaTableName).orElseThrow(() -> new IllegalArgumentException("Metric not found: " + catalogSchemaTableName));
        ...
        {
            return Optional.ofNullable(analyze(analysis, node.getQuery(), sessionContext, analyticsMDL, scope))
        ...
        {
            ExpressionAnalysis expressionAnalysis = ExpressionAnalyzer.analyze(scope, expression, sessionContext, analyticsMDL, analysis);
