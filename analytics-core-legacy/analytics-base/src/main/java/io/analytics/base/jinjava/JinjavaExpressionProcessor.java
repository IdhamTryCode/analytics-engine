...
package io.analytics.base.jinjava;
...
import io.analytics.base.dto.Macro;
import io.analytics.base.macro.Parameter;

import java.util.List;
import java.util.Optional;

import static io.trino.sql.SqlFormatter.formatSql;
import static io.analytics.base.metadata.StandardErrorCode.SYNTAX_ERROR;
import static io.analytics.base.sqlrewrite.Utils.parseExpression;
...
public class JinjavaExpressionProcessor
{
            ...
            if (c == '{') {
                if (i + 1 < source.length() && source.charAt(i + 1) == '{') {
                    ...
                    if (inQuoted || inDoubleQuoted) {
                        ...
                        throw new AnalyticsException(SYNTAX_ERROR, format("Generating macro failed: Nested expression is not supported for macro: %s", source));
                    }
                    else {
                        inExpression = true;
                    }
                ...
                if (i + 1 < source.length() && source.charAt(i + 1) == '}') {
                    ...
                    if (inQuoted || inDoubleQuoted) {
                        ...
                        throw new AnalyticsException(SYNTAX_ERROR, format("Generating macro failed: Unmatched }} in %s", source));
                    }
                    else {
                        inExpression = false;
    ...
    private String processExpression(String expression)
    {
        ...
        return Optional.ofNullable(new Processor(macros, callerInfo).process(macroExpression, null))
                .orElseThrow(() -> new AnalyticsException(SYNTAX_ERROR, format("Failed to apply macro to %s in %s", expression, source)));
    }
