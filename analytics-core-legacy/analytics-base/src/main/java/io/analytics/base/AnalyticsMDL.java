...
package io.analytics.base;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.google.common.collect.ImmutableMap;
import com.hubspot.jinjava.Jinjava;
import io.analytics.base.dto.CacheInfo;
...
import io.analytics.base.jinjava.JinjavaExpressionProcessor;
import io.analytics.base.jinjava.JinjavaUtils;
...
import static io.analytics.base.macro.Parameter.TYPE.MACRO;
import static java.util.Objects.requireNonNull;
import static java.util.function.UnaryOperator.identity;
import static java.util.stream.Collectors.joining;
import static java.util.stream.Collectors.toList;

public class AnalyticsMDL
{
    public static final AnalyticsMDL EMPTY = AnalyticsMDL.fromManifest(Manifest.builder().setCatalog("default").setSchema("default").build());
    ...
    public static AnalyticsMDL fromJson(String manifest)
            throws JsonProcessingException
    {
        return new AnalyticsMDL(MAPPER.readValue(manifest, Manifest.class));
    }

    public static AnalyticsMDL fromManifest(Manifest manifest)
    {
        return new AnalyticsMDL(manifest);
    }

    private AnalyticsMDL(Manifest manifest)
    {
    ...
    private Manifest renderManifest(Manifest original)
    {
        ...
        List<Model> renderedModels = original.getModels().stream().map(model -> {
            List<io.analytics.base.dto.Column> processed = model.getColumns().stream().map(column -> renderExpression(column, macroTags, original)).collect(toList());
            return new Model(
                    model.getName(),
                    model.getRefSql(),
                    model.getBaseObject(),
    ...
    private io.analytics.base.dto.Column renderExpression(io.analytics.base.dto.Column original, String macroTags, Manifest unProcessedManifest)
    {
        ...
        return new io.analytics.base.dto.Column(original.getName(),
                original.getType(),
    ...
    public Optional<String> getColumnType(String objectName, String columnName)
    {
        ...
        if (getModel(objectName).isPresent()) {
            return getModel(objectName).get().getColumns().stream()
                    ...
                    .map(io.analytics.base.dto.Column::getType)
                    .findAny();
        }
