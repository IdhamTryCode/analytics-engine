...
public class ExpressionRelationshipAnalyzer
{
     ...
     * @param mdl analytics mdl
     * @param model the model that expression belongs to
     * @return ExpressionRelationshipInfo
     */
    public static Set<ExpressionRelationshipInfo> getToOneRelationships(Expression expression, AnalyticsMDL mdl, Model model)
    {
     ...
     * @return ExpressionRelationshipInfo
     */
    public static Set<ExpressionRelationshipInfo> getRelationships(Expression expression, AnalyticsMDL mdl, Model model)
    {
            ...
            extends DefaultTraversalVisitor<Void>
    {
        ...
        public RelationshipCollector(AnalyticsMDL analyticsMDL, Model model, boolean allowToManyRelationship)
        {
            this.analyticsMDL = requireNonNull(analyticsMDL);
        ...
        protected Void visitDereferenceExpression(DereferenceExpression node, Void ignored)
        {
            if (node.getField().isPresent()) {
                ...
                if (qualifiedName != null) {
                    Optional<ExpressionRelationshipInfo> expressionRelationshipInfo = createRelationshipInfo(qualifiedName, model, analyticsMDL);
    ...
    public static Optional<ExpressionRelationshipInfo> createRelationshipInfo(QualifiedName qualifiedName, Model model, AnalyticsMDL mdl)
    {
        ...
        for (int i = 0; i < qualifiedName.getParts().size(); i++) {
            ...
            Optional<Column> relationshipColumnOpt = AnalyticsMDL.getRelationshipColumn(current, columnName);
    ...
    private static Relationship getRelationshipFromMDL(Column relationshipColumn, AnalyticsMDL mdl)
    {
    ...
    private static Model getNextModel(Column relationshipColumn, AnalyticsMDL mdl)
    {
