...
import io.analytics.base.dto.DateSpine;
import io.analytics.base.dto.Metric;
import io.analytics.base.dto.Model;
import io.analytics.base.sqlrewrite.analyzer.Field;
import io.analytics.base.sqlrewrite.analyzer.MetricRollupInfo;
import io.analytics.base.sqlrewrite.analyzer.RelationType;
import io.analytics.base.sqlrewrite.analyzer.Scope;
import io.analytics.base.sqlrewrite.analyzer.ScopeAnalysis;
import io.analytics.base.sqlrewrite.analyzer.ScopeAnalyzer;
...
public final class Utils
{
    ...
    public static Query parseCumulativeMetricSql(CumulativeMetric cumulativeMetric, AnalyticsMDL analyticsMDL)
    {
        String sql = getCumulativeMetricSql(cumulativeMetric, analyticsMDL);
        try {
            Statement statement = parseSql(sql);
            if (statement instanceof Query) {
                return (Query) statement;
    ...
    private static String getCumulativeMetricSql(CumulativeMetric cumulativeMetric, AnalyticsMDL analyticsMDL)
    {
        requireNonNull(cumulativeMetric, "cumulativeMetric is null");

        String windowType = getWindowType(cumulativeMetric, analyticsMDL)
    ...
    private static Optional<String> getWindowType(CumulativeMetric cumulativeMetric, AnalyticsMDL analyticsMDL)
    {
        Optional<Model> baseModel = analyticsMDL.getModel(cumulativeMetric.getBaseObject());
        ...
        Optional<Metric> baseMetric = analyticsMDL.getMetric(cumulativeMetric.getBaseObject());
        ...
        Optional<CumulativeMetric> baseCumulativeMetric = analyticsMDL.getCumulativeMetric(cumulativeMetric.getBaseObject());
        if (baseCumulativeMetric.isPresent()) {
            if (baseCumulativeMetric.get().getWindow().getName().equals(cumulativeMetric.getWindow().getRefColumn())) {
                ...
                //  this recursive call happen before cyclic DAG check in AnalyticsSqlRewrite
                return getWindowType(baseCumulativeMetric.get(), analyticsMDL);
            }
            else {
    ...
    // TODO: handle analytics view scope https://github.com/Canner/analytics/issues/338
    public static Scope analyzeFrom(AnalyticsMDL analyticsMDL, SessionContext sessionContext, Relation node, Optional<Scope> context)
    {
        ScopeAnalysis analysis = ScopeAnalyzer.analyze(analyticsMDL, node, sessionContext);
        List<ScopeAnalysis.Relation> usedAnalyticsObjects = analysis.getUsedAnalyticsObjects();
        ImmutableList.Builder<Field> fields = ImmutableList.builder();
        analyticsMDL.listModels().stream()
                .filter(model -> usedAnalyticsObjects.stream().anyMatch(relation -> relation.getName().equals(model.getName())))
                .forEach(model ->
                        model.getColumns().forEach(column -> fields.add(toField(analyticsMDL, model.getName(), column, usedAnalyticsObjects))));

        analyticsMDL.listMetrics().stream()
                .filter(metric -> usedAnalyticsObjects.stream().anyMatch(relation -> relation.getName().equals(metric.getName())))
                .forEach(metric -> {
                    metric.getDimension().forEach(column -> fields.add(toField(analyticsMDL, metric.getName(), column, usedAnalyticsObjects)));
                    metric.getMeasure().forEach(column -> fields.add(toField(analyticsMDL, metric.getName(), column, usedAnalyticsObjects)));
                });
    ...
    private static Field toField(AnalyticsMDL analyticsMDL, String modelName, Column column, List<ScopeAnalysis.Relation> usedAnalyticsObjects)
    {
        ScopeAnalysis.Relation relation = usedAnalyticsObjects.stream()
                .filter(r -> r.getName().equals(modelName))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Model not found: " + modelName));

        return Field.builder()
                .tableName(new CatalogSchemaTableName(analyticsMDL.getCatalog(), analyticsMDL.getSchema(), modelName))
