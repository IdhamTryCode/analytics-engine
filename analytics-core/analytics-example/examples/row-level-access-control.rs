...
use analytics_core::mdl::manifest::{JoinType, Manifest, SessionProperty};
use analytics_core::mdl::{transform_sql_with_ctx, AnalyzedAnalyticsMDL};

/// It's an example to show how to use analytics engine to set up row level access control
...
async fn main() -> datafusion::common::Result<()> {
    ...
    ctx.register_csv(
        "documents",
        "analytics-example/data/company/documents.csv",
        CsvReadOptions::new(),
    )
    .await?;
    let documents = ctx
    ...
    ctx.register_csv(
        "tenants",
        "analytics-example/data/company/tenants.csv",
        CsvReadOptions::new(),
    )
    .await?;
    let tenants = ctx
    ...
    ctx.register_csv(
        "users",
        "analytics-example/data/company/users.csv",
        CsvReadOptions::new(),
    )
    .await?;
    let users = ctx
    ...
    let analyzed_mdl =
        Arc::new(AnalyzedAnalyticsMDL::analyze_with_tables(manifest, register)?);
    ...
    let sql = "select * from analytics.test.documents";
    let sql =
