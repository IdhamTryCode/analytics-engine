...
use analytics_core::mdl::context::{apply_analytics_on_ctx, Mode};
use analytics_core::mdl::manifest::{JoinType, Manifest};
...
async fn main() -> Result<()> {
    ...
    .await?;
    let df = match ctx
        .sql("select totalprice from analyticsai.public.customers")
        .await
    {
        Ok(df) => df,
        Err(e) => {
