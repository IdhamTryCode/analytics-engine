...
load_dotenv()
ANALYTICS_URL = os.getenv("ANALYTICS_URL", "localhost:8000")
USER_AGENT = "analytics-app/1.0"
...
async def make_query_request(sql: str, dry_run: bool = False):
    headers = {"User-Agent": USER_AGENT, "Accept": "application/json", "x-analytics-fallback_disable": "true"}
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"http://{ANALYTICS_URL}/v3/connector/{data_source}/query?dryRun={dry_run}",
                headers=headers,
                json={
                    "sql": sql,
...
async def make_table_list_request():
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"http://{ANALYTICS_URL}/v2/connector/{data_source}/metadata/tables",
                headers=headers,
...
async def make_constraints_list_request():
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                f"http://{ANALYTICS_URL}/v2/connector/{data_source}/metadata/constraints",
                headers=headers,
                json={"connectionInfo": connection_info},
            )
            return response.text
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(
                f"http://{ANALYTICS_URL}/v3/connector/{data_source}/functions",
                headers=headers,
            )
            return response.text
        except Exception as e:
...
async def deploy(mdl: Manifest) -> str:
    global mdl_base64
    """
    Deploy the MDL JSON schema to Analytics Engine
    """
...
async def query(sql: str) -> str:
    """
    Query the Analytics Engine with the given SQL query
    """
...
async def dry_run(sql: str) -> str:
    """
    Dry run the query in Analytics Engine with the given SQL query.
...
@mcp.resource("analytics://metadata/manifest")
async def get_full_manifest() -> str:
    """
    Get the current deployed manifest in Analytics Engine
    """
...
@mcp.resource("analytics://metadata/tables")
async def get_available_tables_resource() -> str:
    """
    Get the available tables in Analytics Engine
    """
...
async def get_table_columns_info(
    ...
    """
    Batch get the column info for the given table and column names in Analytics Engine
...
async def get_table_info(table_name: str) -> str:
    """
    Get the table info for the given table name in Analytics Engine
    """
    mdl = orjson.loads(base64.b64decode(mdl_base64).decode("utf-8"))
...
async def get_column_info(table_name: str, column_name: str) -> str:
    """
    Get the column info for the given table and column name in Analytics Engine
    """
    mdl = orjson.loads(base64.b64decode(mdl_base64).decode("utf-8"))
    # find the specific table
...
async def get_relationships() -> str:
    """
    Get the relationships in Analytics Engine
    """
    mdl = orjson.loads(base64.b64decode(mdl_base64).decode("utf-8"))
...
@mcp.tool()
async def get_analytics_guide() -> str:
    """
    Understand how to use Analytics Engine effectively to query your database
    """

    tips = f"""
    ## Tips for using Analytics Engine with {data_source.capitalize()}
    You are connected to a {data_source.capitalize()} database via Analytics Engine.
    ...
    return f"""
    Analytics Engine is a semantic layer to help you query your database easily using SQL. It supports ANSI SQL to query your database.
    Analytics SQL doesn't equal to your database SQL. Analytics Engine translates your Analytics SQL to your database SQL internally.
    Avoid to use database specific SQL syntax in your Analytics SQL.
    The models you can access are defined in the MDL (Model Definition Language) schema you deployed.

    Here are some tips to use Analytics Engine effectively:
    ...
    5. Avoid to use `LATERAL` statement in your queries, as Analytics Engine may not support it well. Use normal `JOIN` or `CROSS JOIN UNNEST` instead.
    6. Check the tips below for some general tips and some tips specific to the connected DataSource Type.
    {tips}

    ## General Tips
    1. If you encounter any issues, please check the health of Analytics Engine using the `health_check` tool.
...
async def health_check() -> str:
    """
    Check the health of Analytics Engine
    """
    try:
        response = await make_query_request("SELECT 1")
        if response.status_code == 200:
            return "Analytics Engine is healthy"
        else:
            return "Analytics Engine is not healthy"
    except Exception as e:
        return "Analytics Engine is not healthy"
