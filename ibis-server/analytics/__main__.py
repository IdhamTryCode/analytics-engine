...
from analytics import create_session_context
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m analytics <data_source> <mdl_path> [connection_info_path]")  # noqa: T201
        sys.exit(1)
    ...
    if len(sys.argv) > 3:
        with open(connection_info_path := sys.argv[3]) as f:
            ...
            # The connection_info file produced by Analytics AI dbt integration
    ...
    print("You can now interact with the Analytics session using the 'analytics' variable:")  # noqa: T201
    print("> task = analytics.sql('SELECT * FROM your_table').execute()")  # noqa: T201
    print("> print(task.results)")  # noqa: T201
    print("> print(task.formatted_result())")  # noqa: T201
    code.interact(local={"analytics": session})
