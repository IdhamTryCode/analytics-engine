...
if [[ -z "${ANALYTICS_NUM_WORKERS}" ]]; then
    echo "ANALYTICS_NUM_WORKERS is not set. Using default value of 2."
    ANALYTICS_NUM_WORKERS=2
else
    ANALYTICS_NUM_WORKERS=${ANALYTICS_NUM_WORKERS}
fi

echo "Number of workers: ${ANALYTICS_NUM_WORKERS}"
...
# Start the AnalyticsUvicornWorker with the specified configuration
${CMD_PREFIX} gunicorn app.main:app --bind 0.0.0.0:8000 \
    -k app.worker.AnalyticsUvicornWorker \
    --workers ${ANALYTICS_NUM_WORKERS} \
    --max-requests 1000 \
